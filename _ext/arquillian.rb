...
module Awestruct::Extensions::Repository::Visitors
  ...
  module GenericComponent
    ...
    def visit(repository, site)
      ...
      rc.tags.select {|t|
        ...
        release = OpenStruct.new({
          ...
          #:license => 'Apache License 2.0',
          :sha => sha,
          :html_url => build_commit_url(repository, sha, 'html'),
          :json_url => build_commit_url(repository, sha, 'json'),
          :date => commit.date,
          :released_by => OpenStruct.new({
            :name => committer.name,
            :email => committer.email
          }),
          :contributors => resolve_contributors_between(repository, prev_sha, sha),
          :published_artifacts => []
        })
      ...
      # FIXME not dry!
      resolve_contributors_between(repository, prev_sha, rc.revparse('HEAD')).each do |contrib|
        i = c.contributors.index {|n| n.email == contrib.email}
        if i.nil?
          c.contributors << contrib
        else
      ...
      c.last_commit = OpenStruct.new({
        ...
        :sha => last.sha,
        :html_url => build_commit_url(repository, last.sha, 'html'),
        :json_url => build_commit_url(repository, last.sha, 'json')
      })
      c.unreleased_commits = resolve_commits_between(repository, prev_sha, rc.revparse('HEAD')).size
    ...
    def resolve_commits_between(repository, sha1, sha2)
      rc = repository.client
    ...
    def resolve_contributors_between(repository, sha1, sha2)
      author_index = {}
    ...
    def build_commit_url(repository, sha, ext)
